...
func TestOddProgramRepository(t *testing.T) {
	t.Run(`Given a OddProgramRepository, `, func(t *testing.T) {
		ctx := context.Background()
		path := "/tmp/odd_programs.yml"
		populateFile(t, path)
		repo := disk.NewOddProgramRepository(path)
		...
		then it save a odd program`, func(t *testing.T) {
			...
			require.NoError(t, err)
			odds := []program.Odd{
				fixtures.OddProgramBuilder{Hour: &hour, Zones: []string{
					"4",
				}}.Build(),
				fixtures.OddProgramBuilder{Hour: &hour2, Zones: []string{
					"3",
				}}.Build(),
				fixtures.OddProgramBuilder{Zones: []string{
					"1", "2",
				}}.Build(),
			}
